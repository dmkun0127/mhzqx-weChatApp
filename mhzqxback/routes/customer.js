...
router.get("/getopenid", function (req, res, next) {
  ...
  var secret = "9edc872a5b04df73cc5c34f283b93ec1"; //请自主修改自己的小程序secret ,使用的是测试号去https://developers.weixin.qq.com/miniprogram/dev/devtools/sandbox.html登陆查询
  var wx_url =
    ...
    connection.query(customerSql.queryAll, function (err, res) {
      ...
      if (res) {
        ...
        if (!isHave) {
          ...
          connection.query(
            orderSql.selectCusOrder + `'${myCusid}'`,
            function (err, res) {
              if (res) {
                if (res.length > 0) {
                  isHaveOrder = true;
